...
def fetch_rates() -> tuple[Response, Response]:
    ...
    try:
        ecb_rates = httpx.get(
            "https://www.ecb.europa.eu/stats/eurofxref/eurofxref.zip", timeout=30
        )
        ecb_rates.raise_for_status()

        historical_ecb_rates: Response = httpx.get(
            "https://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist.zip", timeout=30
        )
...
def create_html(
    currency_codes: list[str], rates: CurrencyData, mean: CurrencyMean
) -> None:
